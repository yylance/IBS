(function ($Core) {
    ...
    if ($Core.Lang.langKey == undefined) {
        ...
        $Core.Lang.print = '打印';//添加打印按钮（yylance,2018-1-20)
    }
...
(function ($, $Core) {
        ...
        _Internal: {
            ...
            ToolBar: function () {
                ...
                */
                ...
                //添加打印按钮--yylance--2018-1-20
                that.BtnPrint = function () {
                    function Obj() {
                        $Core.BtnBase.call(this);
                        //此处添加打印事件
                        this.onExecute = function (dg) {
                            alert("此处加入打印事件");//内部有前中后事件
                        }
                    }
                    return new Obj();
                }();

                //搜索区隐藏和显示--DY--2018.01.12已测试可用
                that.BtnSearchSwitch = function () {
                    function Obj() {
                        $Core.BtnBase.call(this);
                        this.onBeforeExecute = function (param) { };
                        this.onExecute = function (dg) {
                            if (dg.Search.$target.is(":hidden")==true) {
                                dg.Search.$target.show();
                            }
                            else {
                                dg.Search.$target.hide();
                            }
                        };
                        this.onAfterExecute = function (param) { };
                    }
                    return new Obj();
                }();
    ...
    function _createSearchAreaHtml(dg) {
        ...
        if (dataArray.length > 0 || dg.Search.$target) {
            ...
            dg.Search.$target.hide()//DY 默认不显示
            var form = $("<form>");
    ...
    function _createToolBarHtml(dg) {
        ...
        //yylance(2018-1-20)添加打印按钮
        if (actionKeys.indexOf(',print,') > -1 && !dg.ToolBar.BtnPrint.isHidden) {
            dg.ToolBar.BtnPrint.$target = $('<input class=\"print\" flag=\"btn_print\" type=\"button\"   title="打印"  value=\"\"/>');
            item = $("<a>").append(dg.ToolBar.BtnPrint.$target);
            dg.ToolBar.$target.append(item);
            dg.ToolBar.Items.set("print", dg.ToolBar.BtnPrint);
        }

        //DY--2018.01.12
        if (!dg.ToolBar.BtnSearchSwitch.isHidden) {
            dg.ToolBar.BtnSearchSwitch.$target = $('<input class=\"searchSwitch\" flag=\"btn_search_switch\" type=\"button\"  title="查找" value=\"\"/>');
            item = $("<a>").append(dg.ToolBar.BtnSearchSwitch.$target);
            dg.ToolBar.$target.append(item);
            dg.ToolBar.Items.set("searchSwitch", dg.ToolBar.BtnSearchSwitch);
        }
